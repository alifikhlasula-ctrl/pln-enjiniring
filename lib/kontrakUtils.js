...
export async function generateKontrakPDF(intern, nomorSurat, jsPDFLib) {
  ...
  // Mem-parse string dengan markdown "**teks bold**" dan "_teks italic_" (italic diubah jadi italic font)
  function writeRichText(text, x, startY, maxWidth, justify = true) {
    ...
    paragraphs.forEach(paragraph => {
      ...
      const words = []
      // Pisahkan token berdasarkan spasi, tapi simpan sebagai token utuh
  ...
  y += LH * 2 // Jarak dari header ke paragraf pembuka
  ...
  const renderPasal = (nomor, judul) => {
    if (y + LH * 4 > H - MB) { doc.addPage(); y = MT }
    doc.setFont('helvetica', 'normal')
  ...
  // ────────── TANDA TANGAN ─────────────────────────────────────────────────
  if (y + LH * 8 > H - MB) { doc.addPage(); y = MT }
  y += LH * 2
  const colLeft  = ML + TW * 0.15
  const colRight = ML + TW * 0.85 // geser agar proporsional ke kanan

  doc.setFont('helvetica', 'bold')
  doc.text('PIHAK KEDUA', colLeft, y, { align: 'center' })
  doc.text('PIHAK PERTAMA', colRight, y, { align: 'center' })
  y += LH * 6  // ruang tanda tangan (6 baris)
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    // Render Logo di Kiri Bawah (Footer)
    if (logoBase64) {
      // Skalakan logo agar muat di footer (tinggi max 8mm, lebar auto menyesuaikan proporsi 40:14)
      doc.addImage(logoBase64, 'PNG', ML, footerY - 2, 22.8, 8) 
    } else {
