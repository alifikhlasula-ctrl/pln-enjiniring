...
// ── Core read/write (same contract as old fs-based version) ──
// ── Cached Read Strategy (Vercel Data Cache) ──
const fetchJsonStore = unstable_cache(async () => {
  const record = await prisma.jsonStore.findUnique({ where: { key: STORE_KEY } })
  if (!record) {
