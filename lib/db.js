...
const fetchJsonStore = async (mode = 'ACTIVE') => {
  ...
  entry.inflight = (async () => {
    try {
      ...
      // PROD: Let the error bubble up so the UI knows the DB is busy
      console.error(`[db.js] fetchJsonStore(${storeKey}) error:`, err.message)
      throw err
    } finally {
      entry.inflight = null
    }
  ...
  getEffectiveStatus: (intern) => {
    ...
    if (s === 'ACTIVE' || s === 'PENDING') {
      ...
      if (intern.periodStart) {
        const start = new Date(intern.periodStart); start.setHours(0,0,0,0)
    ...
    if (s === 'PENDING') return 'PENDING' // manual override tetap dihormati
    return s
  }
