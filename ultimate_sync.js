...
async function main() {
  ...
  // Build a lookup map from the Excel text
  const excelMap = {};
  dataListStr.trim().split('\n').forEach(line => {
    const parts = line.split('\t');
    if (parts.length >= 3) {
      excelMap[parts[0].trim().toLowerCase()] = {
        email: parts[1].trim(),
        plneCode: parts[2].trim()
      };
    }
  });
  ...
  for (let i = 0; i < data.interns.length; i++) {
    const intern = data.interns[i];
    if (intern.deletedAt) continue;

    const nameKey = intern.name.toLowerCase().trim();
    const excelInfo = excelMap[nameKey];

    if (!excelInfo) {
      console.log('Skipping (not in excel):', intern.name);
      continue; // Skip if not in the provided excel list
    }

    // Restore their authentic user account
    if (!intern.userId) {
    ...
    if (userIdx === -1) {
      ...
      data.users.push({
        ...
        password: excelInfo.plneCode, // Set password to their PLNE code
        name: intern.name,
        role: 'INTERN'
      });
      created++;
      console.log('Created user for:', intern.name);
    } else {
      // Update existing user strictly to match Excel
      data.users[userIdx].email = excelInfo.email;
      data.users[userIdx].password = excelInfo.plneCode; // Set password to their PLNE code
      
      // Some users had 'password123' previously. Now it will be strictly fixed!
      reset++;
    }
  }
  ...
  console.log(`Final Sync! Created ${created} missing users, Reset ${reset} users to their PLNE passwords.`);
  await p.$disconnect();
}
