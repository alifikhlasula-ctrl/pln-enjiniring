  ...
  return (
    <div className="allowance-page">
      ...
      {data?.status === 'TRANSFERRED' && (
        <div style={{ background: '#e0f2fe', border: '1px solid #7dd3fc', padding: '1rem 1.5rem', borderRadius: 12, marginBottom: '1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            <h4 style={{ color: '#0369a1', fontWeight: 800, display: 'flex', alignItems: 'center', gap: 6 }}>
              <AlertCircle size={18} /> Dana Telah Ditransfer
            </h4>
            <p style={{ color: '#0c4a6e', fontSize: '0.85rem', marginTop: 4 }}>
              Admin mengkonfirmasi bahwa uang saku Anda sudah ditransfer pada <strong>{new Date(data.transferredAt).toLocaleDateString('id-ID')}</strong>. Mohon pastikan dan klik konfirmasi jika dana sudah masuk ke rekening Anda.
            </p>
          </div>
          <button className="btn btn-primary" style={{ background: '#0284c7' }} onClick={handleConfirm} disabled={loading}>
            <CheckCircle2 size={16} /> Konfirmasi Dana Diterima
          </button>
        </div>
      )}
                 ...
                 <div className="grid grid-cols-3 gap-4">
                    ...
                    <div style={{ background:'rgba(0,0,0,0.15)', padding:'1rem', borderRadius:'12px', border:'1px solid rgba(255,255,255,0.1)' }}>
                       ...
                       <p style={{ fontSize:'0.85rem', fontWeight:900, background: data?.status === 'PAID' ? '#10b981' : data?.status === 'TRANSFERRED' ? '#0284c7' : '#f59e0b', padding:'3px 8px', borderRadius:6, textAlign:'center' }}>
                         {data?.status === 'PAID' ? 'DITERIMA (PAID)' : data?.status === 'TRANSFERRED' ? 'DITRANSFER' : 'PENDING'}
                       </p>
                    </div>
                 </div>
              </div>
              ...
              {data?.status === 'PAID' && (
                <div style={{ marginTop:25, borderTop:'1px solid var(--border)', paddingTop:20, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                   ...
                   <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                      ...
                      <div>
                        <p style={{ fontSize:'0.85rem', fontWeight:800, color:'var(--primary)' }}>Waktu Transfer Admin</p>
                        <p style={{ fontSize:'0.7rem', color:'var(--text-muted)' }}>Pada {data.transferredAt ? new Date(data.transferredAt).toLocaleDateString('id-ID') : '-'}</p>
                      </div>
                   </div>
                </div>
              )}
